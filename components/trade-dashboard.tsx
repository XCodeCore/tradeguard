...
export function TradeDashboard({ developmentPreview }: { developmentPreview: boolean }) {
  ...
  const verdict = assessment?.status ?? (sample ? "REDUCE_SIZE" : null);
  const amountCopy = amountPresentation(assessment, imported?.request.proposedNotional);
  ...
  return <main className="terminal">
    <header className="topbar"><div className="brand"><Shield/><div><strong>TradeGuard</strong><span>AI Pre-Trade Safety Firewall</span></div></div><div className="connection workflow"><Dot/>Agent OS via Codex</div></header>
    <div className="frame">
