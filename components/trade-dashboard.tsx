...
function hasMarketDrivenWarning(assessment: Assessment | undefined, proposedNotional: number | undefined): boolean {
  return assessment?.status === "REDUCE_SIZE" && assessment.saferAmount !== null && proposedNotional !== undefined && assessment.saferAmount >= proposedNotional && assessment.overrides.minimumStatus === "REDUCE_SIZE" && assessment.overrides.reasons.length > 0;
}
...
export function TradeDashboard({ developmentPreview }: { developmentPreview: boolean }) {
  ...
  const marketDrivenWarning = hasMarketDrivenWarning(assessment, imported?.request.proposedNotional);
    ...
    <div className="frame">
      ...
      <div className="workspace">
        ...
        <section className="assessment">
          <article className="panel hero"><div className="section-head"><div><p className="eyebrow">Deterministic assessment</p><h2>TradeGuard Assessment</h2></div><code>{imported ? "IMPORTED MCP" : sample ? "SAMPLE" : "AWAITING DATA"}</code></div><div className="score-area"><div className={`score-ring ${score !== null ? "filled" : ""}`} style={score !== null ? { "--score": `${score}%` } as CSSProperties : undefined}><div><strong>{score === null ? "—" : Math.round(score)}</strong><span>/ 100</span></div></div><div className="verdict"><b>{classification(score)}</b><p>Final verdict</p><h3>{displayStatus(verdict)}</h3><div><span>{marketDrivenWarning ? "Size-based amount" : "Safer suggested amount"}</span><strong>{assessment?.saferAmount != null ? `${money(assessment.saferAmount)} USDT` : sample ? "$620 USDT" : "—"}</strong></div>{marketDrivenWarning && <p className="market-warning">No further size reduction was calculated. The remaining warning is driven by current market conditions: {assessment?.overrides.reasons.join("; ")}.</p>}</div></div></article>
